...
### 步骤6：导演评估阶段（LLM调用2）
**流程位置**：`6. 导演评估阶段` → `DirectorEvaluator.evaluate_as_director`

**LLM交互**：**主要LLM调用（导演评估）**

**重要**：**这是一个LLM调用，包含两部分工作**：
1. **环境变化分析（第一部分）**：分析Agent响应对环境的影响，以及Agent响应的实际执行情况
2. **决策制定（第二部分）**：基于环境变化分析结果，决定是否触发事件、出现怪物、转换场景

**关键说明**：
- **环境变化分析和决策制定在同一个LLM调用中完成**
- **在同一个prompt中，LLM需要先完成环境变化分析，然后基于分析结果进行决策制定**
- **返回格式需要同时包含环境变化分析结果和决策结果**
- **输入信息**：所有信息都在同一个prompt中提供给LLM，支持两部分工作

**流程定位**：
- **Agent负责"产生想法"**：Agent根据观察到的信息产生想法和决策
- **导演评估负责"环境变化分析+决策制定"**（在同一个LLM调用中完成）：
  - **环境变化分析（第一部分）**：分析Agent的想法/决策的**实际执行情况**（基于环境因素，如遇到障碍、怪物逃跑等）和对环境的影响
  - **决策制定（第二部分）**：基于环境变化分析结果，决定是否触发事件、让怪物出现、转换场景

**环境变化分析的职责**（第一部分）：
- **分析Agent的想法/决策对环境的影响**：Agent的想法/决策可能会改变环境（如移动改变位置、战斗改变场景状态、场景中的什么东西遭到破坏等）
- **分析Agent的想法/决策的实际执行情况**：Agent的想法/决策可能因为环境因素而实际执行结果不同（如想移动但遇到障碍、想战斗但怪物已逃跑等）
- **生成执行结果**：根据环境因素，生成Agent的想法/决策的实际执行结果
- **注意**：环境变化分析**不会决定怪物出现**，怪物出现由导演评估的决策制定部分决定

**流程顺序**：
1. Agent响应生成（考虑自身状态+玩家指令+当前环境）
2. **导演评估（LLM调用2，一个LLM调用包含两部分工作）**：
   - **第一步：环境变化分析**：在同一个LLM调用中，先分析Agent响应对环境的影响，以及实际执行情况
   - **第二步：决策制定**：基于第一步的环境变化分析结果，决定是否触发事件、出现怪物、转换场景。导演评估负责决定事件触发和统一停止点
   - **重要**：这两步在同一个LLM调用中顺序完成，先完成环境变化分析，然后基于分析结果进行决策制定
...
**重要说明**：
- **所有信息都在同一个prompt中提供给LLM**，支持环境变化分析和决策制定两部分工作
- **导演评估可以看到完整信息**，包括技术性标识符和"里"信息
- **LLM需要在同一个调用中先完成环境变化分析，然后基于分析结果进行决策制定**
...
5. **【Agent的想法/决策】**（最重要，用于环境变化分析和决策制定）
   - 每个Agent的想法/决策（Agent的响应文本，角色的想法和决策）
   - **第一步：环境变化分析**：分析Agent的想法/决策对环境的影响，以及实际执行情况（成功/失败、失败原因、实际结果等）
   - **第二步：决策制定**：基于环境变化分析结果来决定事件和场景变化
...
11. **【你的任务】**（在同一个LLM调用中完成两部分工作）
    - **第一步：环境变化分析**：仔细分析Agent响应对环境的影响，以及Agent响应的实际执行情况（成功/失败、失败原因、实际结果等）
    - **第二步：决策制定**：基于第一步的环境变化分析结果，评估是否触发事件、让怪物出现（怪物出现后作为环境的一部分，状态保存在环境状态中，不再独立维护角色卡）、转换场景/房间
    - **重要**：这两步在同一个LLM调用中完成，先完成环境变化分析，然后基于分析结果进行决策制定
...
14. **【导演工作流程】**（在同一个LLM调用中完成）
    - 导演的工作步骤（这两步在同一个LLM调用中顺序完成）
    - **第一步：环境变化分析**：分析Agent响应对环境的影响，以及实际执行情况（成功/失败、失败原因、实际结果等）
    - **第二步：决策制定**：基于第一步的环境变化分析结果，评估是否触发事件、让怪物出现、转换场景
    - **重要**：在同一个prompt中，先完成环境变化分析，然后基于分析结果进行决策制定
...
**重要**：返回格式需要同时包含环境变化分析结果和决策结果，因为这是一个LLM调用。

```json
{
    "environment_analysis": {
        "updated_scene_description": "更新后的场景描述（文本格式）",
        "scene_state_changes": {
            "location": "新位置",
            "time": "时间变化",
            "weather": "天气变化"
...
**字段说明**：
- **environment_analysis**：环境变化分析的结果（第一部分）
  - `updated_scene_description`：更新后的场景描述
  - `scene_state_changes`：场景状态变化（位置、时间等）
  - `agent_execution_results`：Agent执行结果列表
- **director_decision**：导演决策的结果（第二部分）
  - `trigger_event`：触发的事件ID
  - `event_description`：事件描述
  - `appear_monster`：出现的怪物名称
  - `monster_description`：怪物出现描述
  - `transition_target`：场景转换目标
  - `transition_type`：转换类型
  - `elapsed_time`：消耗的时间
  - `reasoning`：决策理由（需说明环境变化分析过程和基于分析结果的决策）

**字段对应关系**：

| 输入信息 | 输出字段 | 说明 |
|---------|---------|------|
| **【Agent实际响应】**（最重要） | `environment_analysis` | **第一步：环境变化分析**，分析Agent响应对环境的影响和实际执行情况 |
| **【Agent实际响应】** | `director_decision.trigger_event` | **第二步：决策制定**，基于环境变化分析结果，从事件列表中选择要触发的事件ID |
| **【Agent实际响应】** | `director_decision.event_description` | **第二步：决策制定**，基于环境变化分析结果，生成事件描述 |
| **【Agent实际响应】** | `director_decision.appear_monster` | **第二步：决策制定**，基于环境变化分析结果（如提到战斗、遭遇），从怪物列表中选择要出现的怪物名称 |
| **【Agent实际响应】** | `director_decision.monster_description` | **第二步：决策制定**，基于环境变化分析结果，生成怪物出现描述 |
| **【Agent实际响应】** | `director_decision.transition_target` | **第二步：决策制定**，基于环境变化分析结果（如提到移动、到达、进入），从可连接目标中选择转换目标ID |
| 【当前场景下的可能事件】 | `director_decision.trigger_event` | 从事件列表中选择要触发的事件ID（需结合环境变化分析结果） |
| 【当前场景下的可能事件】 | `director_decision.event_description` | 基于事件的`description_template`生成（需结合环境变化分析结果） |
| 【潜在怪物列表】 | `director_decision.appear_monster` | 从怪物列表中选择要出现的怪物名称（需结合环境变化分析结果） |
| 【潜在怪物列表】 | `director_decision.monster_description` | 基于怪物的`battle_description_template`生成（需结合环境变化分析结果） |
| 【可连接目标】 | `director_decision.transition_target` | 从可连接目标中选择转换目标ID（需结合环境变化分析结果） |
| 【可连接目标】 | `director_decision.transition_type` | 对应目标的类型（scene/room） |
| 【当前时间信息】 | `director_decision.elapsed_time` | 根据停留时间和操作复杂度估算 |
| 【你的任务】、【原则】 | `director_decision.reasoning` | 基于任务和原则生成决策理由（需说明环境变化分析过程和基于分析结果的决策） |
...
**环境状态更新（导演评估后）**：
- **环境变化分析结果**（从返回的`environment_analysis`中获取）：更新环境状态（位置、时间等，基于Agent行为的影响）
- **决策制定结果**（从返回的`director_decision`中获取）：
  - 如果触发事件，会更新环境状态（记录已触发事件）
  - 如果出现怪物，会更新环境状态（怪物作为环境的一部分，状态保存在环境状态中）
  - 如果转换场景，会更新环境状态（更新场景ID、房间ID等）
...
2. **导演评估阶段**（步骤6，LLM调用2，最重要）
   - **输入**：Agent响应、当前场景内容、更新后的Agent状态和环境状态、事件列表（**包含"里"信息**）、怪物列表（**包含"里"信息**）、连接目标（**包含技术性标识符**）、时间信息等
   - **输出**：**一个JSON对象，包含两部分**
     - **environment_analysis**：环境变化分析结果（场景变化描述、Agent执行结果等）
     - **director_decision**：决策结果（事件触发、怪物出现、场景转换、时间消耗等）
   - **作用**：**这是一个LLM调用，在同一个prompt中完成两部分工作**
     - **环境变化分析（第一部分）**：分析Agent响应对环境的影响，以及Agent响应的实际执行情况
     - **决策制定（第二部分）**：基于环境变化分析结果，决定事件触发、怪物出现、场景转换。**导演评估负责决定事件触发和统一停止点，确保所有Agent在同一时刻停止**
   - **对应文档**：本文档步骤6说明
...
2. Agent响应生成（LLM调用1）
   ...
   - 输出：Agent响应（包含预期状态变化）
    ↓
3. 导演评估阶段（LLM调用2，一个LLM调用包含两部分）
   - 第一部分：环境变化分析
     - 分析Agent响应对环境的影响
     - 分析Agent响应的实际执行情况
     - 生成环境变化分析结果（包含在返回的`environment_analysis`中）
   - 第二部分：决策制定
     - 基于环境变化分析结果（在同一个LLM调用中完成）
     - 输入：环境变化分析结果（LLM内部使用）、事件列表（包含"里"信息）、怪物列表、连接目标（包含技术性标识符）
     - 生成决策结果（包含在返回的`director_decision`中）
   - 返回：包含`environment_analysis`和`director_decision`的JSON对象
   - 更新环境状态：
     - 从`environment_analysis`更新位置、时间等
     - 从`director_decision`更新事件、怪物、场景转换等
    ↓
4. 更新Agent实际状态
...
| **环境变化分析结果** | `appear_monster` | **第二步：决策制定**，基于环境变化分析结果（如提到战斗、遭遇），从怪物列表中选择要出现的怪物名称（怪物出现后状态保存在环境状态中，不再独立维护角色卡） |
...
| 【潜在怪物列表】 | `appear_monster` | 从怪物列表中选择要出现的怪物名称（需结合环境变化分析结果，怪物出现后状态保存在环境状态中） |
...
2. Agent响应生成（LLM调用1）
   ...
   ↓
3. 导演评估（LLM调用2，一个LLM调用包含两部分）
   - 第一部分：环境变化分析
     - 分析Agent响应对环境的影响
     - 分析Agent响应的实际执行情况
     - 生成环境变化分析结果（返回在`environment_analysis`中）
   - 第二部分：决策制定
     - 基于环境变化分析结果（在同一个LLM调用中完成）
     - 决定事件触发、怪物出现、场景转换
     - 生成决策结果（返回在`director_decision`中）
   - 更新环境状态：
     - 从`environment_analysis`更新位置、时间等
     - 从`director_decision`更新事件、怪物、场景转换等
   ↓
...
- **导演评估后**（一个LLM调用，包含两部分结果）：
  - **从返回的`environment_analysis`中**：分析Agent响应对环境的影响，以及Agent响应的实际执行情况，更新环境状态（位置、时间等）
  - **从返回的`director_decision`中**：如果触发事件或转换场景，会更新环境状态（事件、怪物、场景转换）
...
9. **信息完整性**：导演评估阶段需要完整的信息才能做出正确决策，**在同一个LLM调用中**，导演评估会先进行环境变化分析，然后基于分析结果进行决策

10. **LLM调用方式**：**环境变化分析和决策制定在同一个LLM调用中完成**，在同一个prompt中，LLM需要先完成环境变化分析，然后基于分析结果进行决策制定，返回格式包含`environment_analysis`和`director_decision`两部分

11. **时间同步**：时间信息必须准确，因为会影响事件触发条件的判断

12. **状态更新**：每次LLM调用后，需要及时更新状态，确保下次调用时信息是最新的

11. **表里区分**：**Agent只能看到"表"信息（玩家可见），不能看到"里"信息（隐藏的游戏机制）**，这是保持游戏沉浸感的关键
